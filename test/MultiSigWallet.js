...
contract('MultiSigWallet', (accounts) => {
    ...
    describe('fallback function', async () => {
        ...
        it('should receive GIV', async () => {
            let token = await GiveToken.new();
        ...
        [
            ...
            context(`with ${spec.owners.length} owners and requirement of ${spec.requirement}`, async () => {
                ...
                beforeEach(async () => {
                    ...
                    token = await GiveToken.new();

                    await token.mint(wallet.address, initGIVBalance);
                    await token.endMinting();
                    assert.equal((await token.balanceOf(wallet.address)).toNumber(), initGIVBalance);
                });
