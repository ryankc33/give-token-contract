...
const GiveToken = artifacts.require('../contracts/GiveToken.sol');
const GiveTokenSaleMock = artifacts.require('./helpers/GiveTokenSaleMock.sol');
...
contract('GiveTokenSale', (accounts) => {
    const MINUTE = 60;
    ...
    // This represents the USD price per one GIV, such MAX_TOKENS_SOLD * GIV_PER_USD is the $75M cap.
    const GIV_PER_USD = 6829 * TOKEN_UNIT;
    const GIV_PER_WEI = new BigNumber(GIV_PER_USD).div(WEI_PER_USD).floor().toNumber();
    ...
    const HUNDRED_BILLION_GIV = Math.pow(10, 11) * TOKEN_UNIT;

    const GIV_TOKEN_GRANTS = [
        {grantee: '0x56ae76573ec54754bc5b6a8cbf04bbd7dc86b0a0', value: 60 * HUNDRED_BILLION_GIV, startOffset: 0, cliffOffset: 0, endOffset: 3 * YEAR, installmentLength: 1 * DAY, percentVested: 0},
        {grantee: '0x3bf4bbe253153678e9e8e540395c22bff7fca87d', value: 30 * HUNDRED_BILLION_GIV, startOffset: 0, cliffOffset: 0, endOffset: 120 * WEEK, installmentLength: 12 * WEEK, percentVested: 100}
    ];

    const PRESALE_TOKEN_GRANTS = [
        {grantee: '0xebfbfbdb8cbef890e8ca0143b5d9ab3fe15056c8', value: 2 * HUNDRED_BILLION_GIV, startOffset: 0, cliffOffset: 1 * YEAR, endOffset: 1 * YEAR, installmentLength: 1 * DAY, percentVested: 50},
        {grantee: '0x499d16bf3420f5d5d5fbdd9ca82ff863d505dcdd', value: 2 * HUNDRED_BILLION_GIV, startOffset: 0, cliffOffset: 1 * YEAR, endOffset: 1 * YEAR, installmentLength: 1 * DAY, percentVested: 50},
        {grantee: '0x06767930c343a330f8f04680cd2e3f5568feaf0a', value: 1 * HUNDRED_BILLION_GIV, startOffset: 0, cliffOffset: 1 * YEAR, endOffset: 1 * YEAR, installmentLength: 1 * DAY, percentVested: 50},
    ...
    const GRANTS = GIV_TOKEN_GRANTS.concat(PRESALE_TOKEN_GRANTS);
    ...
    describe('construction', async () => {
        ...
        it('should not allow to initialize with null funding recipient address', async () => {
            await expectRevert(GiveTokenSaleMock.new(null, now + 100));
        });

        it('should not allow to initialize with 0 funding recipient address', async () => {
            await expectRevert(GiveTokenSaleMock.new(0, now + 100));
        });

        it('should be initialized with a future starting time', async () => {
            await expectRevert(GiveTokenSaleMock.new(fundRecipient, now - 100));
        });

        it('should be initialized with a derived ending time', async () => {
            let startTime = now + 100;
            let sale = await GiveTokenSaleMock.new(fundRecipient, startTime);

            assert.equal((await sale.endTime()).toNumber(), startTime + (await sale.SALE_DURATION()).toNumber());
        });

        it('should deploy the GiveToken contract and own it', async () => {
            let sale = await GiveTokenSaleMock.new(fundRecipient, now + 100);
            assert(await sale.Give() != 0);

            let token = GiveToken.at(await sale.give());
        ...
        it('should deploy the VestingTrustee contract and own it', async () => {
            ...
            assert.equal(await trustee.give(), token.address);
        ...
        it('should initialize token grants', async () => {
            let sale = await GiveTokenSaleMock.new(fundRecipient, now + 100);
            assert.equal((await sale.getTokenGranteesLength()).toNumber(), GIV_TOKEN_GRANTS.length);

            for (const giveTokenGrant of GIV_TOKEN_GRANTS) {
                await testTokenGrantExists(sale, giveTokenGrant);
            }
        });

        it('should be ownable', async () => {
            let sale = await GiveTokenSaleMock.new(fundRecipient, now + 10000);
    ...
    describe('addTokenGrant', async () => {
        ...
        beforeEach(async () => {
            sale = await GiveTokenSaleMock.new(fundRecipient, now + 1000);
        });
    ...
    describe('grantTokens', async () => {
        ...
        beforeEach(async () => {
            start = now + 1000;
            sale = await GiveTokenSaleMock.new(fundRecipient, start);
        ...
        context('after the sale', async () => {
            ...
            it('should grant Give tokens to all non pre-sale grantees', async () => {
                await grantTokens(sale, GIV_TOKEN_GRANTS);
            });
            ...
            context('with pre-sale grants', async () => {
                ...
                it('should grant Give and pre-sale tokens to all pre-sale grantees', async () => {
    ...
    describe('finalize', async () => {
        ...
        beforeEach(async () => {
            ...
            token = GiveToken.at(await sale.give());
            ...
            context('with pre-sale grants', async () => {
                ...
                describe('vesting', async () => {
                    ...
                    let calcVestedTokens = async (grant) => {
                        ...
                        // Amount of tokens actually sold = sale.tokensSold = just 1 GIVE
                        //
                        // Token distribution is 30% Give, 60% GIVE Foundation, 10% Participants (including pre-sale).
                        // Since only 1 GIVE was sold and accounts to 10% of total tokens issued, this means total
                        // tokens \ issued is 10 GIVE.
                    ...
                    beforeEach(async () => {
                        ...
                        // Verify that both the GiveTokenSale and the VestingTrustee share the same GiveToken.
                        assert.equal(token.address, await trustee.give());
                    });
                    ...
                    it('should grant tokens', async () => {
                        for (const grant of GRANTS) {
                            ...
                            console.log(`\texpecting ${vestedTokens.vested / TOKEN_UNIT} vested GIV...`);
                            console.log(`\texpecting ${vestedTokens.transferred / TOKEN_UNIT} issued GIV...`);
    ...
    let verifyTransactions = async (sale, fundRecipient, method, transactions) => {
        ...
        for (const t of transactions) {
            ...
            let tokens = new BigNumber(t.value.toString()).mul(GIV_PER_WEI);

            console.log(`\t[${++i} / ${transactions.length}] expecting account ${t.from} to buy up to ` +
                `${tokens.toNumber() / TOKEN_UNIT} GIV for ${t.value / TOKEN_UNIT} ETH`
            );
            ...
            let participantGIVBalance = await token.balanceOf(t.from);
            ...
            // Test for correct participant ETH, GIV balance, and total tokens sold:
            ...
            tokens = contribution.mul(GIV_PER_WEI);
            ...
            contribution = tokens.div(GIV_PER_WEI);
            ...
            // Test for correct participant ETH + GIV balances.
            ...
            // GIV:
            assert.equal((await token.balanceOf(t.from)).toNumber(), participantGIVBalance.plus(tokens).toNumber());
    ...
    describe('transfer ownership', async () => {
        ...
        // GIV token contract ownership transfer tests.
        let testTransferAndAcceptTokenOwnership = async () => {
            let owner = accounts[0];
            let newOwner = accounts[1];
            let notOwner = accounts[8];

            describe('Give token contract ownership transfer', async () => {
                describe('request', async () => {
                    it('should allow contract owner to request transfer', async () => {
                        assert.equal(await token.owner(), sale.address);

                        await sale.requestGiveTokenOwnershipTransfer(newOwner, {from: owner});
                    });

                    it('should not allow non-owner to request transfer', async () => {
                        await expectRevert(sale.requestGiveTokenOwnershipTransfer(newOwner, {from: notOwner}));
                    });
                });
                ...
                describe('request and accept', async () => {
                    it('should transfer ownership to new owner', async () => {
                        ...
                        // anymore).
                        await expectRevert(sale.requestGiveTokenOwnershipTransfer(newOwner, {from: owner}));
                    });
                    ...
                    it('should be able to claim ownership back', async () => {
                        ...
                        await sale.acceptGiveTokenOwnership({from: owner});
                        assert.equal(await token.owner(), sale.address);
                    });
                });
            });
