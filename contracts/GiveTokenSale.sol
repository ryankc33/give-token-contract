...
/// @title Give token sale contract.
contract GiveTokenSale is Ownable, TokenHolder {
    ...
    // Give token unit.
    // Using same decimal value as ETH (makes ETH-GIV conversion much easier).
    // This is the same as in Give token contract.
    ...
    // GIV to 1 USD ratio,
    // such that MAX_TOKENS_SOLD / GIV_PER_USD is the $75M cap.
    uint256 public constant GIV_PER_USD = 6829 * TOKEN_UNIT;

    // GIV to 1 wei ratio.
    uint256 public constant GIV_PER_WEI = GIV_PER_USD / WEI_PER_USD;
    ...
    address public constant GIV_FOUNDATION_ADDRESS = 0x56aE76573EC54754bC5B6A8cBF04bBd7Dc86b0A0;
    address public constant GIV_ADDRESS = 0x3bf4BbE253153678E9E8E540395C22BFf7fCa87d;
    ...
    function GiveTokenSale(address _fundingRecipient, uint256 _startTime) {
        require(_fundingRecipient != address(0));
        require(_startTime > now);

        // Deploy new GiveToken contract.
        give = new GiveToken();

        // Deploy new VestingTrustee contract.
        trustee = new VestingTrustee(give);
    ...
    function initTokenGrants() private onlyOwner {
        // Issue the remaining 60% to Give Foundation's multisig wallet. In a few days, after the token sale is
        // finalized, these tokens will be loaded into the GiveVestingTrustee smart contract, according to the white
        // paper. Please note, that this is implied by setting a 0% vesting percent.
        tokenGrantees.push(GIV_FOUNDATION_ADDRESS);
        tokenGrants[GIV_FOUNDATION_ADDRESS] = TokenGrant(MAX_TOKENS.mul(60).div(100), 0, 0, 3 years, 1 days, 0);

        // GIV, 30%
        tokenGrantees.push(GIV_ADDRESS);
        tokenGrants[GIV_ADDRESS] = TokenGrant(MAX_TOKENS.mul(30).div(100), 0, 0, 120 weeks, 12 weeks, 100);
    }

    /// @dev Adds a Give token vesting grant.
    ...
    /// @dev Deletes a Give token grant.
    ...
    function create(address _recipient) public payable onlyDuringSale {
        ...
        uint256 weiLeftInSale = MAX_TOKENS_SOLD.sub(tokensSold).div(GIV_PER_WEI);
        ...
        uint256 tokensToIssue = weiToParticipate.mul(GIV_PER_WEI);
        if (tokensLeftInSale.sub(tokensToIssue) < GIV_PER_WEI) {
            // If purchase would cause less than GIV_PER_WEI tokens left then nobody could ever buy them.
    ...
    function issueTokens(address _recipient, uint256 _tokens) private {
        // Request Give token contract to mint the requested tokens for the buyer.
        give.mint(_recipient, _tokens);
    ...
    /// @dev Requests to transfer control of the Give token contract to a new owner.
    /// @param _newOwnerCandidate address The address to transfer ownership to.
    ///
    /// NOTE:
    ///   1. The new owner will need to call Give token contract's acceptOwnership directly in order to accept the ownership.
    ///   2. Calling this method during the token sale will prevent the token sale to continue, since only the owner of
    ///      the Give token contract can issue new tokens.
    function requestGiveTokenOwnershipTransfer(address _newOwnerCandidate) external onlyOwner {
        give.requestOwnershipTransfer(_newOwnerCandidate);
    }

    /// @dev Accepts new ownership on behalf of the Give token contract.
    // This can be used by the sale contract itself to claim back ownership of the Give token contract.
    function acceptGiveTokenOwnership() external onlyOwner {
        give.acceptOwnership();
    }
