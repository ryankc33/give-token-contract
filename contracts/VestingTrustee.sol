...
/// @title Vesting trustee contract for Give token.
contract VestingTrustee is Ownable {
    ...
    /// @dev Constructor that initializes the address of the Give token contract.
    /// @param _give GiveToken The address of the previously deployed Give token contract.
    function VestingTrustee(GiveToken _give) {
        require(_give != address(0));
    ...
    function grant(address _to, uint256 _value, uint256 _start, uint256 _cliff, uint256 _end,
        ...
        require(totalVesting.add(_value) <= give.balanceOf(address(this)));
    ...
    function revoke(address _holder) public onlyOwner {
        ...
        give.transfer(msg.sender, refund);
    ...
    function unlockVestedTokens() external {
        ...
        give.transfer(msg.sender, transferable);
